...
혼자 개발하는 환경에는 테스트의 빈틈을 막아줄 리뷰어가 없다.
사람의 기준은 마감이 닥치면 흔들리지만, 에이전트의 시스템 프롬프트는 흔들리지 않는다.
그래서 리뷰어의 역할을 같은 신념 9개를 공유하는 에이전트 쌍 — 작성하는 writer, 감사하는 reviewer — 으로 시스템화해, 테스트 품질의 하한선을 사람의 의지가 아닌 코드로 강제한다.
...
- [persona-suite](https://github.com/xxx-sj/persona-suite) — 개발 라이프사이클 앞단(요구사항 정의 → 데이터모델/코드 설계 → 테스트 리스트업 → 프로덕션 리스크 감사)을 맡는 자매 플러그인. 이 에이전트 쌍은 그 다음 단계인 구현·테스트에서 쓰인다.
